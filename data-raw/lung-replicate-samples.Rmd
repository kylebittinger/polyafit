...
lungreplicate_samples <- read_tsv("emily_full_reproducability_map_rev1.txt", show_col_types = FALSE) %>%
  ...
  mutate(subject_id = if_else(
    str_starts(subject_id, "Pulm"), 
    str_replace(subject_id, "^Pulm", "H"),
    paste0("Tx", subject_id))) %>%
  mutate(sample_type = str_replace(sample_type, "OralWash", "Oral wash")) %>%
  mutate(sample_type = str_replace(sample_type, "BAL", "Lung")) %>%
...
lungreplicate_reads <- read_tsv("otu_table_nochimeras.txt", skip = 1, show_col_types = FALSE) %>%
  rename(otu_id = `#OTU ID`, lineage = `Consensus Lineage`)
lungreplicate_taxa <- lungreplicate_reads %>%
  select(otu_id, lineage)
lungreplicate_reads <- lungreplicate_reads %>%
  select(-lineage) %>%
  pivot_longer(-otu_id, names_to = "sample_id", values_to = "reads")
lungreplicate_taxa <- lungreplicate_taxa %>%
...
Remove extra samples from lungreplicate_reads
...
lungreplicate_reads <- lungreplicate_reads %>%
  ...
  ungroup()
lungreplicate_taxa <- lungreplicate_taxa %>%
  filter(otu_id %in% lungreplicate_reads$otu_id) %>%
  filter(!is.na(Phylum))
lungreplicate_reads <- lungreplicate_reads %>%
  filter(otu_id %in% lungreplicate_taxa$otu_id)
```
...
props <- lungreplicate_reads %>%
  left_join(lungreplicate_taxa, b = "otu_id") %>%
...
```{r}
usethis::use_data(lungreplicate_samples)
usethis::use_data(lungreplicate_reads)
usethis::use_data(lungreplicate_taxa)
```
